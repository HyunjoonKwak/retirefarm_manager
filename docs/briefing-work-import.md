...
- 실제 Work 화면 JSON의 sources 객체와 숫자 시계열을 확인했다. 가격·포장 범위와 행 구조를 검증하며 알려지지 않은 필드는 원문으로 보존한다.
- `/reports` Work 보관함과 `supplemental-sources.ts` 연결 완료. 보고 종료일이 가장 최근인 보관본(동일 종료일 최신 저장본)에서 해당 보고 주간의 공개 평균을 참조한다. 보고서별 명시적 원본 선택은 후속 작업이다.
