...
| 4 | 결과 생성은 됐지만 complete 전달 확인 불가 | 아래 "전달 불명확" 참고 |
...
5. 전용 임시 디렉터리(0700)를 만들고 출력 스키마 파일을 쓴 뒤 Codex 실행. 프롬프트는 stdin으로만 전달.
...
8. `complete` 전송. 성공 응답(`ok:true`)이 없으면 같은 본문을 최대 3회까지 재전송 (서버 멱등). 401/409/400은 재전송하지 않음.
...
`complete` 응답을 끝내 받지 못하면 워커는 **Codex를 다시 실행하지 않고, fail도 보내지 않은 채** 종료합니다. 서버 lease가 만료되면 서버가 작업을 다시 큐에 넣을 수 있고, 같은 lease·inputHash·result의 재전송은 서버가 멱등 처리합니다.

이때 임시 디렉터리는 삭제되지 않고 로그에 `workDir` 경로가 남습니다. 안에는 출력 스키마·전용 지침·최종 메시지·`undelivered-result.json`(검증 통과한 결과)이 있으며 인증 토큰은 없습니다. 확인 후 직접 삭제하십시오.
