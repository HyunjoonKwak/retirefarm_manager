...
상품 주소는 직접 링크 또는 같은 카드의 판매처 링크와 상품 번호를 조합한 제안이다. 조합 주소와 광고 이동 주소 항목은 실제 상품 페이지에서 주소를 확인해야 한다. 광고 표시를 발견하면 광고로 유지하고, 표시를 찾지 못한 항목도 비광고로 단정하지 않는다. 품목 확인과 저장할 후보 선택은 앱에서 진행한다. 구매 표기는 설명으로, 검색 리뷰 수는 집계 기준 미확인으로 보존한다.

현재 위치는 시장 전체 순위가 아니라 해당 화면의 상품 목록 순서다. 검색 주소의 추적 파라미터를 제거하고 검색어·정렬·페이지 정보를 보존한다. 개인화·배송지 등 검색 환경은 미확인으로 남긴다. 일반 가격비교(`/search/all`)에서는 네이버플러스 검색으로 이동하라는 안내를 제공한다. 목록 구조를 인식하지 못하면 빈 성공 결과를 만들지 않고 오류로 알린다.

기존 설치는 소스 폴더를 새 ZIP 내용으로 교체한 뒤 `chrome://extensions`에서 새로고침한다. 추가 권한은 없으며 현재 탭에서 버튼을 누를 때만 수집한다. 자동으로 검색어를 바꾸거나 주간 예약을 실행하지 않는다.

검색 출처 URL은 검색어·정렬·페이지 외 파라미터를 제거합니다. 필터·개인화·배송지 상태 전체를 재현하지 않으며 검색 환경은 미확인으로 표시합니다.
