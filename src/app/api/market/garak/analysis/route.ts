...
export async function GET(request: NextRequest) {
  ...
  const { productName, days, origin, unit } = parsed.data;
  const asOf = new Date();
  const since = new Date(asOf);
  since.setDate(since.getDate() - days); since.setHours(0, 0, 0, 0);
  try {
    const rows = await prisma.auctionResult.findMany({
      where: { productName, auctionDate: { gte: since }, ...(origin ? { origin: { contains: origin } } : {}), ...(unit ? { unit } : {}) },
    ...
    return NextResponse.json({
      ...
      scope: { productName, days, origin: origin || null, unit: unit || null },
