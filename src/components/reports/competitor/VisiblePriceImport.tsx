...
export interface VisiblePriceDraft { price: string; shippingFee: string; availability: Parsed["availability"]; notes: string }
...
export function VisiblePriceImport({ entryId, optionLabel, busy, onApply }: {
  entryId: string; optionLabel: string; busy: boolean; onApply: (draft: VisiblePriceDraft) => void;
}) {
  ...
  const extract = () => {
    const result = parseVisiblePage(text);
  ...
  return <details className="rounded-md border p-3 text-sm space-y-2">
    ...
    <Textarea id={id("text")} value={text} rows={5} maxLength={30000} disabled={busy} placeholder={"선택 옵션: 중과 2kg\n총 금액 18,500원\n무료배송"}
      onChange={e => { setText(e.target.value); setParsed(null); setConfirmed(false); setApplied(false); }} />
    <Button type="button" variant="outline" size="sm" disabled={busy || !text.trim()} onClick={extract}>가격 읽기</Button>
    {parsed && <div className="space-y-2">
      ...
      <Button type="button" size="sm" disabled={busy || !matches || !confirmed} onClick={() => {
        onApply({ price: parsed.price === null ? "" : String(parsed.price), shippingFee: parsed.shippingFee === null ? "" : String(parsed.shippingFee),
          availability: parsed.availability, notes: `화면 텍스트 확인 (${VISIBLE_PAGE_PARSER_VERSION}) · 옵션: ${selectedOption.trim()} · 추출 상품가 ${won(parsed.price)} · 추출 배송비 ${won(parsed.shippingFee)} · 수량 1, 조건부 쿠폰 제외 확인. ${parsed.hints.join(" ")}`.slice(0, 1000) });
        setApplied(true);
      }}>기록 양식에 채우기</Button>
