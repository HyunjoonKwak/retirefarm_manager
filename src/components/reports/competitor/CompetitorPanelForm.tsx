...
export function CompetitorPanelForm({ initial, busy, fromCandidate, onSubmit, onReset }: Props) {
  ...
  const update = (patch: Partial<PanelDraft>) => { setDraft(current => ({ ...current, ...patch })); setError(""); };
  ...
  return <form className="rounded-lg border p-4 space-y-3" onSubmit={submit} aria-labelledby="competitor-panel-heading">
    ...
    <label className="flex items-start gap-2 text-sm">
      <Checkbox checked={draft.confirmed} disabled={busy} onCheckedChange={value => update({ confirmed: value === true })} aria-label="상품 페이지에서 직접 확인함" />
      <span>위 URL의 상품 페이지에서 상품·옵션명과 중량을 직접 확인했습니다. (검색 결과 추정값을 그대로 저장하지 않습니다)</span>
    </label>
