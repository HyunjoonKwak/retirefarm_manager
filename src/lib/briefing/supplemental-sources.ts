...
export async function supplementalSources(userId: string, start: Date, end: Date, now: Date) {
  ...
  sources.push({ id: "competitors", title: "직접 검증한 고정 경쟁점 옵션 가격", url: null,
    ...
    note: `보고서 생성 시각 기준. 지난 보고 주간과 별개인 최신 참고 자료입니다. 활성 점포 ${competitors.activeCount}/30곳. 최근 7일 내 재고·배송비와 품종·크기 기준·중량을 확인한 동일 상품군에서 3곳 이상일 때 배송 포함 중앙가격을 산출합니다. API 검색 최저가는 이 지표에 포함하지 않습니다.` });
  ...
  limitations.push("경쟁점 최신 참고 가격은 사용자가 확인한 옵션·일반 배송비 기준이며 지역 추가운임·개별 쿠폰 조건을 보장하지 않습니다. 크기 기준 미확인 자료는 대표 가격에서 제외하며, 브라우저 검색 순서는 비광고 순위가 아닙니다.");
  if (imported) {
